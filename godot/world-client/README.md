...
当前角色采用程序化人形，可表现步行、慢跑、排队、坐下、饮用、使用设施和相向交谈。左键拖动
旋转导演镜头，右键、`Shift + 左键` 或中键拖动平移，滚轮缩放；`WASD`/方向键沿地面移动，
`Shift` 加速，`Home` 返回总览，`F` 返回已选居民。单击居民仍会选中并平滑跟随。
Web 视口使用宿主容器的真实宽高，不维持固定 16:9 比例，因此嵌入导演台时不会产生左右黑边。
客户端还会按权威世界时间
