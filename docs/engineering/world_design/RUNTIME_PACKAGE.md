...
- `world.yml` 缺失：视为严重错误
- `rules.yml` 缺失：第一阶段可允许回退为空规则集
- `policies/default.yml` 缺失：允许回退到平台默认 policy
- `constitution.md` 缺失：允许为空，但给 warning
...
同时保留现有单文件 helper 作为兼容层，但新逻辑尽量基于 package。
