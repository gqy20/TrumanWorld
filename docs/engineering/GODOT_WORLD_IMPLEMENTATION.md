...
- 类型：`engineering`
- 状态：`in_progress`（Phase 0～3 与 Phase 4 具身展示切片已实现）
...
截至 2026-08-03，Phase 0～3 与 Phase 4 具身展示切片已完成：
...
尚未进入正式世界页。Phase 4 仍缺少 Portal 传播、同行、主动加入活动、室内外切换与更完整的
多人会话编排；这些能力不能由客户端自行补造权威状态。当前程序化人形和建筑属于玩法验证资产，
后续可在协议不变的前提下替换为正式角色骨骼、动画树和模块化场景资产。
...
- 增加 `stage.renderer: godot` 配置。
- 同一场景支持 `voxel` 和 `godot` 双渲染器。
- 先对开发 Run 和测试 Scenario 开启。
- 观察稳定后才设为默认。
