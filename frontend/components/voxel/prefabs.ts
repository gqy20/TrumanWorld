...
type PrefabWriter = (
  ...
  options?: Pick<VoxelPrefabBlock, "castShadow" | "receiveShadow">,
) => void;
...
function buildHomePrefab(add: PrefabWriter, plotItem: VoxelPlot): void {
  ...
  addSteppedRoof(add, plotItem, height, "roofRed");
  addFrontDoorAndWindows(add, plotItem, height);
  add([0.42, height + 0.36, 0.16], [0.14, 0.48, 0.14], "wood");
}
...
function buildCafePrefab(add: PrefabWriter, plotItem: VoxelPlot): void {
  ...
  addSteppedRoof(add, plotItem, height, "roofRed");
  addFrontDoorAndWindows(add, plotItem, height);
  add([0, 0.88, front + 0.16], [1.16, 0.14, 0.34], "white");
...
function buildLibraryPrefab(add: PrefabWriter, plotItem: VoxelPlot): void {
  ...
  add([0, height + 0.14, 0], [plotItem.footprint.width + 0.18, 0.22, plotItem.footprint.depth + 0.12], "roofBlue");
...
function buildGenericPrefab(add: PrefabWriter, plotItem: VoxelPlot): void {
  const height = 1.08;
  addBuildingBase(add, plotItem, height, "wallWarm");
  addSteppedRoof(add, plotItem, height, "roofGreen");
  addFrontDoorAndWindows(add, plotItem, height);
}
...
function buildPlazaPrefab(add: PrefabWriter, plotItem: VoxelPlot): void {
  ...
  add([0, 0.22, 0], [1.12, 0.2, 1.12], "wallStone");
  add([0, 0.35, 0], [0.84, 0.08, 0.84], "water", { castShadow: false });
  add([0, 0.72, 0], [0.18, 0.74, 0.18], "white");
  add([0, 1.08, 0], [0.38, 0.14, 0.38], "wallStone");
  for (const [x, z] of [
    [-0.92, -0.92],
    [0.92, -0.92],
    [-0.92, 0.92],
...
function addSteppedRoof(
  ...
  add([0, height + 0.12, 0], [plotItem.footprint.width + 0.12, 0.18, plotItem.footprint.depth + 0.08], material);
  add([0, height + 0.29, 0], [plotItem.footprint.width * 0.74, 0.16, plotItem.footprint.depth * 0.72], material);
  add([0, height + 0.43, 0], [plotItem.footprint.width * 0.42, 0.12, plotItem.footprint.depth * 0.4], material);
}
